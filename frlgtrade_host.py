...
Trainer identity is intentionally configured in ``frlgsim.host_profile``.
This entry point only translates command-line options into ``HostRunConfig``.
...
from frlgsim.host_app import HostApplication, HostRunConfig  # noqa: E402
...
def _offered_slots(parser, args):
    try:
        explicit = parse_slots(args.slots, args.trades, len(args.party))
...
def main(argv=None):
    ...
    if not 1 <= len(args.party) <= 6:
        parser.error(f"supply 1..6 party mons; got {len(args.party)}")
    if args.trades > len(args.party):
        parser.error(f"--trades {args.trades} needs at least that many party mons")
    if os.geteuid() != 0:
        parser.error("live LDN hosting requires root; run with sudo -E")

    config = HostRunConfig(
        party_paths=tuple(args.party),
        output_path=args.out,
        output_size=args.out_size,
        output_format=args.out_format,
        trade_slot=args.slot,
        offered_slots=_offered_slots(parser, args),
        trades=args.trades,
    ...
    joined = HostApplication(config, DEFAULT_TRAINER, log=_Log(args.verbose)).run()
